...
const initializeCache = (): void => {
  ...
  try {
    ...
    if (stored) {
      ...
      console.log(
        "Testimonials: Loaded from localStorage:",
        testimonialsCache.length
      );
    } else {
      testimonialsCache = [];
    ...
    try {
      ...
      if (stored) {
        ...
        if (JSON.stringify(storedData) !== JSON.stringify(testimonialsCache)) {
          console.log(
            "Testimonials: Detected cross-context change via polling"
          );
          ...
          window.dispatchEvent(
            new CustomEvent("testimonialsUpdated", {
...
const syncAcrossContexts = (testimonials: Testimonial[]) => {
  try {
    ...
    setTimeout(() => {
      window.dispatchEvent(
        new CustomEvent("testimonialsForceSync", {
...
if (typeof window !== "undefined") {
  ...
  if (typeof BroadcastChannel !== "undefined") {
    try {
      ...
      console.log(
        "Testimonials: BroadcastChannel listener setup failed:",
        error
      );
    }
  }
  ...
  // Force sync listener
  window.addEventListener("testimonialsForceSync", (event: Event) => {
    const customEvent = event as CustomEvent;
    if (customEvent.detail.timestamp > lastSyncTime) {
      console.log("Testimonials: Received force sync");
      testimonialsCache = customEvent.detail.testimonials;
      lastSyncTime = customEvent.detail.timestamp;
      localStorage.setItem(STORAGE_KEY, JSON.stringify(testimonialsCache));
...
export const saveTestimonialsIntoDB = async (
...
export const getDisplayedTestimonials = (): Testimonial[] => {
  ...
  console.log(
    "Testimonials: getDisplayedTestimonials returning",
    displayed.length,
    "testimonials"
  );
  return displayed;
...
export const addTestimonial = async (
  ...
  const newTestimonial: Testimonial = {
    ...
    id: newId,
    avatar:
      testimonialData.avatar || testimonialData.author.charAt(0).toUpperCase(),
  };
...
export const updateTestimonial = async (
  ...
  const updatedTestimonial = {
    ...
    avatar:
      updates.avatar ||
      updates.author?.charAt(0).toUpperCase() ||
...
export const listenForTestimonialsUpdates = (
  ...
  const handleCustomEvent = (event: CustomEvent) => {
    console.log(
      "Testimonials: Received update via custom event, source:",
      event.detail.source
    );
